...
class MovieCard extends lit_element_s {
  ...
  btnEvents() {
    /* eslint-disable */;
    oo_oo(), console.log(this.API.url, this.searching, `476ac1f_0`);
  ...
  generateSearchMovies(genres) {
    ...
    this.API.getAllData().then(movies => {
      if (movies.results.length == 0) {
        notiflix_aio_3_2_6_min_default().Notify.failure("sorry, not found");
      }
...
;// CONCATENATED MODULE: ./src/components/root_user.js





class C_Root_U extends lit_element_s {
  static get styles() {
    return [i`
        :host {
          display: block;
...
;// CONCATENATED MODULE: ./src/scripts/user.js
